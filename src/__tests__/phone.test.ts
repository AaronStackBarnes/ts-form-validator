...
describe('formatPhoneUS', () => {
  ...
  it('returns original string if not 10 or 11 digits', () => {
    expect(formatPhoneUS('1234567')).toBe('1234567');
  });
});
...
describe('validatePhone edge cases', () => {
  ...
  it('rejects loose format that does not match regex', () => {
    expect(validatePhone('abc-def-ghij').valid).toBe(false);
  });
});
